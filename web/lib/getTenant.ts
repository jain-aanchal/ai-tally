...
/**
 * Whether a control-plane write that could not reach the gateway may still be ECHOED back to the
 * caller as though it had been accepted (CTO-393).
 *
 * Only off the product path. The echo exists so a fresh clone with no infra still works, and the dev
 * escape hatch is exactly what marks that case: `TALLY_DEV_TENANT` is the only way the dashboard
 * serves a tenant with no authenticated Clerk organization behind it. `sampleDataAllowed()` in
 * lib/mock.ts is a strict subset of this (it requires an explicit demo build on top), so this is the
