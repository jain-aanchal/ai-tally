...
- **Decisions are written with `async_insert`, directly, not through the in-memory burst buffer.** Each decision request makes one insert with `async_insert = 1` and `wait_for_async_insert = 1`. ClickHouse queues concurrent small inserts on the server and flushes them as one part per table per flush, on a size threshold or a busy timeout, so part creation follows the flush timer rather than the request rate. That server-side batching is the fix for part explosion.
- **Why decisions skip the burst buffer (Review 5).** An earlier draft put the burst buffer in front of `async_insert`. The two do not combine:
  - The burst buffer exists to acknowledge a client *before* writing. A durable acknowledgement means waiting for the write, so a request would sit through the buffer's queue, its drain poll (50 ms) and the `async_insert` flush, and the queue wait grows under exactly the load the buffer is meant to absorb.
  - The buffer drains round-robin across tenants in 2,000-row chunks, so what reaches ClickHouse is a mixed chunk, not a client's batch. Insert-time deduplication works per insert query, so no batch-derived token could ever match a redelivered chunk.
  - The burst buffer stays for spans, where acknowledge-then-write is the intended trade until the Kafka backing it was designed for makes that path durable.
- **What "durable acknowledgement" guarantees, precisely.** With `wait_for_async_insert = 1`, ClickHouse confirms only after the rows are written to a part on disk, and the gateway answers the edge only then. That survives a gateway crash: an edge that never received an answer retries. It does **not** survive losing the ClickHouse node's disk. The droplet runs a single, non-replicated ClickHouse, so durability is single-node until ClickHouse is replicated (ClickHouse Cloud, or `ReplicatedMergeTree`).
- **A gateway crash delays a retry; it does not double or lose it.** The Postgres batch claim (0032) is taken when a request arrives. If the gateway dies before recording the outcome, the claim stays `in_flight` for its lease (`IN_FLIGHT_LEASE_S`, 300 seconds), and retries inside the lease are answered retryable. Long-running edges keep the batch within their byte cap and succeed once the lease passes. Serverless edges make one flush attempt, so they record `flush_deadline` and carry `acted` and `error` rows to the next warm invocation. A shorter lease for decision batches is open (14).
- **The flush timer has to fit the serverless deadline.** A decision insert waits for the next flush. With ClickHouse's default 200 ms busy timeout, a quiet tenant's insert waits out most of the timer, which leaves little of the 300 ms serverless flush deadline, so quiet periods would shed the most. Decision inserts therefore set a lower `async_insert_busy_timeout_ms` per query (50 ms as the starting point). The trade is more parts, one per flush, so the value is set from a measured part rate against the table's `parts_to_throw_insert` limit before P0, not assumed.
- **Counts are written per batch, not folded in gateway memory.** Each decision batch carries its per-minute count deltas and inserts them in the same request as its rows; the `SummingMergeTree` sums them at merge, and reads use `sum()`. An earlier draft folded counts in memory and wrote one row per key per minute. That lost the current minute's counts on a gateway crash, for batches already durably acknowledged, which would have left allow verdicts (counted, rarely logged) with no durable record at all. And a batch whose row insert failed after its counts were folded would have been counted again on retry. `async_insert` already combines these small inserts into one part per flush, so folding saved nothing. The edge still merges counts in place while it queues (below); that is safe because merged deltas travel under a batch id and its claim.
...
- The gateway's side is designed to fit inside that deadline through the lowered flush timeout above. When ClickHouse is slower than that, the deadline wins: the edge stops waiting and carries the rows, rather than keeping the function alive.

**Deduplication cost.** Retries must not become a stream of identical rows for ClickHouse to collapse.

- **Retried batches never reach ClickHouse.** Decision batches use the same `(tenant_id, batch_id)` claim in Postgres as `/v1/batches` (migration 0032): a retry of a batch already written is answered from the recorded response, before any insert. This is the primary guard, and it works on the ClickHouse version the stack runs today.
- **Insert-time deduplication is a second layer, and only from ClickHouse 26.2.** The stack runs `clickhouse-server:24.8`. Before 26.2, asynchronous inserts are not deduplicated by default (a separate `async_insert_deduplicate` setting governed it), and `insert_deduplication_token` did not take effect for async inserts. From 26.2, `deduplicate_insert` covers asynchronous inserts per originating query, including non-replicated tables once `non_replicated_deduplication_window` is set above its default of 0. Because decisions skip the burst buffer, one request is one insert query, so a token of `(tenant_id, batch_id)` identifies exactly one batch's rows and a re-sent batch is dropped at insert time. On 24.8 the token is sent but not relied on.
- **`ReplacingMergeTree` covers the residue:** a gateway that dies after the ClickHouse write and before the Postgres record, whose retry after the lease writes the batch again. From 26.2 the token drops that re-send at insert as well, so the merge-time residue shrinks to effectively nothing.
...
| POST | `/v1/governance/decisions` | edge | Decision batches: same `(tenant_id, batch_id)` idempotency as `/v1/batches`, written with `async_insert` and a durable acknowledgement, not through the in-memory burst buffer (8.3) |
...
| `async_insert` for decision ingest (8.3) | Decision ingest in P0 | Not used anywhere today |
| Buffered span ingest: the burst buffer or `async_insert` on `/v1/batches` (8.3) | Protects span ingest from "too many parts", independent of governance | Buffer built but off by default and not enabled on the droplet; Kafka backing not built |
| ClickHouse 26.2 or later (8.3) | Insert-time deduplication of async inserts as a second layer | Stack runs 24.8; the Postgres batch claim is the guard until then |
| Replicated ClickHouse (8.3) | Durability beyond a single node | Single non-replicated node on the droplet |
...
- **Ingest pressure lands on one ClickHouse.** Decisions multiply write volume by the number of capabilities. `async_insert` with a durable acknowledgement and per-batch count deltas (8.3) keep part creation proportional to the flush timer rather than traffic, but a deployment that turns governance on before buffering its span ingest will hit "too many parts" on span ingest first.
...
17. **A shorter `in_flight` lease for decision batches** than the 300 seconds spans use, so a gateway crash delays decision retries for less time, at the cost of a wider window in which a slow-but-alive gateway can be overlapped by a retry.
18. **When to upgrade ClickHouse to 26.2 or later** for insert-time deduplication of async inserts, and when to replicate it for durability beyond one node.

## 15. Phasing

### P0: settings, shadow, decision log

Scope: section 3 settings and audit, with a missing capability row reading as off; resolution that checks the org switch before any degraded branch; edge identities (9.1), including deployment credentials; signed manifests, the root key and the signing key set; bundle compile and distribute; the decision log with rule and attempt in its key, the allow counts table and allow sampling; decision ingest with one `async_insert` per request and a durable acknowledgement, not through the burst buffer; per-batch count deltas; a lowered `async_insert_busy_timeout_ms` for decision inserts, set from a measured part rate; insert deduplication tokens sent, and relied on once ClickHouse is on 26.2 or later; separate byte-capped decision queues with priority shedding at the edge; the feature tag cardinality cap (8.3); mode resolution in the SDK and proxy, with the effective-mode precedence for existing guardrail rules (3.2); the existing guardrail rule kinds and budgets evaluated in **shadow only**; the Governance settings page and shadow overview.
...
- **db/clickhouse:** `governance_decisions.sql` (with `non_replicated_deduplication_window`, effective for async inserts from ClickHouse 26.2) and `governance_decision_counts.sql`; `async_insert` settings for the ingest user; add both ReplacingMergeTree tables to the `FINAL` guard in `web/lib/clickhouse.test.ts` (the counts table is a SummingMergeTree and is read with `sum()`).
- **infra/gateway:** `governance_settings.py`, `governance_bundle.py` (compile, sign, content-addressed routing tables), `governance_sigpack.py` (publish gate: RE2 compile check, true-positive and false-positive corpora, recorded false-positive rate), `governance_leases.py` (ledger, minimum lease, `budget_leased_out`, ephemeral leases with heartbeat reclaim), `governance_seed.py` (settled-watermark seeding, one-way reconcile), `governance_signing.py` (manifests, root and signing key set; the gateway has no signing facility today), `governance_edge_identity.py` (9.1), `governance_decisions.py` (direct `async_insert` with a durable acknowledgement, per-batch count deltas, a lowered busy timeout, insert deduplication tokens), `feature_tag_cardinality.py` (per-tenant distinct-tag cap, `__overflow__`, id-shaped tag detection), `governance_kill.py` (manual switch, runaway proposals, expiring auto-activation); routes in `app.py`; tests including bundle signature, lease invariant (outstanding leases never exceed remaining budget), and resolution-order conformance fixtures.
...
**Review 5** (correctness pass on section 8.3):

- **Decisions skip the burst buffer.** A durable acknowledgement cannot sit behind a buffer that acknowledges before writing, and the buffer's round-robin chunks across tenants could never match a per-batch deduplication token. Decisions now make one `async_insert` per request and rely on ClickHouse's server-side batching.
- **Counts are per-batch deltas, not folded in gateway memory.** Folding lost acknowledged counts on a gateway crash and double-counted a retried batch; `async_insert` already coalesces the small inserts.
- **Insert-time deduplication is scoped to ClickHouse 26.2 or later.** The stack runs 24.8, where async inserts are not deduplicated by default and the token did not take effect; the Postgres batch claim remains the guard until the upgrade.
- **Durability and crash recovery stated precisely:** single-node until ClickHouse is replicated, and a gateway crash delays retries for the 300-second claim lease rather than losing or doubling them.
- **Flush timer fitted to the serverless deadline:** decision inserts lower the busy timeout from the 200 ms default, set from a measured part rate.
