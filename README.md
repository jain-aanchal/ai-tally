...
[![Watch the Demo Video](https://github.com/jain-aanchal/ai-tally/blob/main/Thumbnail.png)](https://www.youtube.com/watch?v=r4NlWZ6eaZI)
...
- **Analyze:** Cost, now a unified explorer (CTO-239): one group-by (layer / feature / model / provider / agent / account) plus the filter bar and a search box drive the headline tiles, the interactive chart, and a breakdown table, with budget-vs-actual and the burn-down forecast alongside. The former Features and Agents tabs fold into it: group-by=feature narrows to one feature to show its economics (cost/user, value/user, payback, value-event config), group-by=agent narrows to one agent to show its run distribution and outlier runs, and `/features` and `/agents` redirect into the explorer. Then Compare (cross-provider replay + eval), Attribution (`$/conversion` and margin per provider), Unit Economics (CAC / LTV / payback), Cost per Customer (direct + allocated cost and margin per account), and Recoverable Cost (recoverable spend across the five detector categories).
...
- **A global filter bar (URL-synced).** Time range (7 / 30 / 90 days / custom), a group-by dimension (layer / feature / model / provider / agent / account), and multi-select dimension filters that narrow the tiles, chart and breakdown together. State lives in the query string, so a filtered view is shareable and the browser back button walks the filter history. Windows are ClickHouse-clock derived, never the Node clock, now that they are user-selectable (CTO-203/226).
